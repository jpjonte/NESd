      ...
      final sub = eventBus.stream
          ...
          .listen((event) {
            if (event is FrameNesEvent || event is DebuggerNesEvent) {
              // do not decode while fast-forwarding to reduce work
              final fast = ref.read(nesStateProvider)?.fastForward ?? false;

              if (fast) {
                // drop any pending decode and keep showing last rendered frame
                final old = pending.value;
