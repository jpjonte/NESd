...
vec4 sampleSource(vec2 texelCenter) {
  vec2 clamped = clamp(texelCenter, vec2(0.5), uSourceRect.zw - 0.5);

  return texture(uSource, (uSourceRect.xy + clamped) / uSourceSize);
}

void main() {
  vec2 uv = FlutterFragCoord().xy / uOutputSize;
  vec2 texel = uv * uSourceRect.zw - 0.5;

  vec2 base = floor(texel) + 0.5;
  vec2 f = texel - floor(texel);

  // sharp bilinear
  vec2 scale = max(floor(uOutputSize / uSourceRect.zw), vec2(1.0));
