...
void main() {
  ...
  Future<FakeNesIsolateHandle> loadWith({required bool lowPassFilter}) async {
    ...
    when(() => settings.lowPassFilter).thenReturn(lowPassFilter);
  ...
  test('loadRom sends an enabled low pass filter to the worker', () async {
    final handle = await loadWith(lowPassFilter: true);

    final sent = handle.sentCommands
        .whereType<SetLowPassFilterCommand>()
        .single;

    expect(sent.enabled, isTrue);
  });

  test('loadRom sends a disabled low pass filter to the worker', () async {
    final handle = await loadWith(lowPassFilter: false);
